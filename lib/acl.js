...
(function(){
  ...
  var PAGE_POLICY = {
    ...
    // PM-884: lead+ (member data, comms, partner/employer ops, analytics) —
    // member tier is the day-to-day operating set only (tasks, calendar,
    // content, podcast, documents, home). Everything touching member data,
    // money-adjacent analytics or external comms needs lead+.
    'crm':            'lead',
    'brief':          'lead',
    'broadcast':      'lead',
    'active-users':   'lead',
    ...
    'ambassadors':    'lead',   // PM-975: area grant (partners) opens it for team — Cole/Calum
    'perks':          'lead',   // PM-975: same — discount-partner catalogue
    'inbound':        'team',
    'employers':      'lead',
    'platform-health': 'lead',  // PM-977 watchtower board
    'app-health':     'lead',
    'usage':          'lead',
    'retention':      'lead',
    'activity-depth': 'lead',
    ...
    // Member+ (day-to-day operating set)
    // home, tasks, calendar, documents, content, podcast (omitted = member)
  };
  ...
  // ---------- Role accessors ----------
  // ---------- PM-907: per-person areas + admin view-as preview ----------
  var VIEW_AS_KEY = 'vyve.cc.viewAs';
  var viewAsRow = null; // admin_users row of the viewed person, loaded by loadViewAs()

  function viewAsEmail(){
    if (serverRole() !== 'admin') return null; // only admins may preview
  ...
  function setViewAsRow(row){ viewAsRow = row || null; }
  function viewingAs(){ return viewAsEmail() ? (viewAsRow || { email: viewAsEmail() }) : null; }

  // The persona all visibility decisions run under.
  // Self by default; the viewed person when an admin has view-as active.
  function effective(){
    var va = viewAsEmail();
    if (va && viewAsRow && viewAsRow.email === va) {
      return { email: va, serverRole: viewAsRow.role || 'team', areas: viewAsRow.areas || null };
    }
    var self = (window.VYVE_SUPABASE && window.VYVE_SUPABASE.cachedAdmin) ? window.VYVE_SUPABASE.cachedAdmin() : null;
    return {
      email: currentEmail(),
      serverRole: serverRole(),
      areas: self ? (self.areas || null) : null
    };
  }

  function hasArea(area){
    var e = effective();
    if (e.serverRole === 'admin') return true;      // admins see everything
    if (!area) return true;                          // untagged items (home) always visible
    if (area === 'system') return false;             // settings stays admin-only
    return !!(e.areas && e.areas.indexOf(area) >= 0);
  }

  // Visibility for a full nav item (slug pages AND href links).
  function canSeeItem(item){
    if (!item) return false;
    if (item.area !== undefined || item.href) return hasArea(item.area);
    return canSeePage(item.slug);
  }

  function role() {
    // PM-907: while an admin previews another person, rank follows the viewed person.
    var va = viewAsEmail();
    if (va) return (viewAsRow && viewAsRow.role === 'admin') ? 'owner' : 'member';
  ...
  function canSeePage(slug) {
    // PM-907: an explicit area grant opens the page regardless of rank.
    var nav = window.VYVE_NAV || [];
    for (var si=0; si<nav.length; si++){
      var its = nav[si].items || [];
      for (var ii=0; ii<its.length; ii++){
        if (its[ii].slug === slug && its[ii].area !== undefined) {
          var e907 = effective();
          if (e907.serverRole !== 'admin' && e907.areas) return hasArea(its[ii].area);
        }
      }
    }

...
(function(){
  ...
  // ===== ACTION PLANS =====
  // Reads flat cache written by the Action Plans page on every render. Each entry:
  // {id, member_id, member_name, position, text, status, due, progress, notes, updated_at}
  var Actions = {
    all: function(){ return load('vyve_action_plans_full', []); },
    byMember: function(memberId){ return this.all().filter(function(a){ return a.member_id === memberId; }); },
    overdue: function(){
      var today = startOfDay();
    ...
    dueSoon: function(days){
      days = days || 14;
      var today = startOfDay();
      var end = endOfDay(addDays(today, days));
      return this.all().filter(function(a){
        if (a.status === 'done') return false;
        var d = parseDate(a.due);
    ...
    inProgress: function(){ return this.all().filter(function(a){ return a.status === 'in-progress'; }); },
    blocked: function(){ return this.all().filter(function(a){ return a.status === 'blocked'; }); },
    completionPct: function(){
      var all = this.all();
  ...
  function recentActivity(n){
    ...
    push(Actions.all().filter(function(a){ return a.updated_at; }), 'action', function(a){ return (a.member_name||'') + ' · ' + (a.text||'').slice(0,80); }, 'updated_at');
    return items.sort(function(a,b){ return b.date - a.date; }).slice(0, n);
  }

  // ---- Three-pillar attention mix ----
  // Counts items with a "pillar" tag (physical/mental/social) across tasks,
  // sessions, content, and action plans for a recency window.
  // Returns: { physical: n, mental: n, social: n, untagged: n, total: n }
  function pillarMix(days){
    days = days || 7;
    var now = Date.now();
    var since = now - days * 86400000;
    var counts = { physical: 0, mental: 0, social: 0, untagged: 0, total: 0 };
    function bump(p){
      var k = (p || '').toLowerCase().trim();
      if (k === 'physical' || k === 'mental' || k === 'social') counts[k]++;
      else counts.untagged++;
      counts.total++;
    }
    // Tasks updated within window
    (Tasks.all() || []).forEach(function(t){
      var d = parseDate(t.updated_at || t.created_at);
      if (d && d.getTime() >= since && (t.status||'') !== 'done') bump(t.pillar);
    });
    // Sessions in window (upcoming or recent)
    (Sessions.all() || []).forEach(function(s){
      var d = parseDate(s.date);
      if (d && Math.abs(d.getTime() - now) <= days*86400000) bump(s.pillar);
    });
    // Content updated within window
    (Content.all() || []).forEach(function(c){
      var d = parseDate(c.updated_at || c.created_at);
      if (d && d.getTime() >= since) bump(c.pillar);
    });
    // Action plan items, recent activity
    (Actions.all() || []).forEach(function(a){
      var d = parseDate(a.updated_at || a.created_at);
      if (d && d.getTime() >= since && (a.status||'') !== 'done') bump(a.pillar);
    });
    return counts;
  }

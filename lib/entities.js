...
(function(){
  ...
  var ENTITIES = {
    action: {
      ...
      get: function(id){
        return safeOne(function(){
          ...
          for (var i=0;i<arr.length;i++) { var r=arr[i]; if (String(r.id) === String(id) || String(r._id) === String(id)) return r; }
          return null;
        });
      },
      titleOf: function(r){
    ...
    deal: {
      ...
      titleOf: function(r){ return r.name || r.company || r.title || 'Untitled deal'; },
      subOf: function(r){
        var v = Number(r.value) || 0;
        var stage = r.stage || '';
        var close = r.expected_close || r.close_date || '';

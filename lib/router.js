...
(function(){
  ...
  var DEFAULT    = 'home';   // PM-755: Home launchpad replaces brief
  var pageCache  = {};
  var current    = null;
  ...
  function resolvePage(slug) {
    // PM-764: domain landing pages — #/domain-{key} all render pages/domain.html,
    // which reads the key from the hash and tiles that section's nav items.
    if (slug.indexOf('domain-') === 0 && window.VYVE_DOMAINS && window.VYVE_DOMAINS[slug.slice(7)]) {
      return { slug: slug, label: window.VYVE_DOMAINS[slug.slice(7)].label, isHub: false, file: 'domain' };
    }
    var hub = lookupTop(slug);
    if (hub && slug !== 'home') return { slug: slug, label: hub.label, isHub: true };
  ...
  function renderTopNav() {
    ...
    var html = visible.map(function(tab){
      ...
      if (tab.href) {
        return '<a class="topnav-tab" href="' + tab.href + '">' +
          icon +
  ...
  function setActiveTop(slug) {
    var topSlug = (window.VYVE_ROUTE_TO_TOP && window.VYVE_ROUTE_TO_TOP[slug]) || 'home';
  ...
  function renderCrumbs(slug) {
    ...
    // Don't render crumbs on Home — keeps the launchpad clean
    if (slug === 'home' || !pageEntry) {
      host.innerHTML = '';
      return;
    }

    var parts = [];
    parts.push('<a class="crumb" href="#/home">VYVE</a>');
  ...
  function renderSidebar() {
    ...
    window.VYVE_NAV.forEach(function(section){
      ...
      var items = section.items.filter(function(item){
        if (!window.VYVE_ACL) return true;
        // PM-907: canSeeItem covers slug pages AND href links via area grants.
        if (window.VYVE_ACL.canSeeItem) return window.VYVE_ACL.canSeeItem(item);
  ...
  function loadPage(slug) {
    ...
    if (slug !== 'app-health' && slug !== 'usage' && slug !== 'retention' && slug !== 'activity' && slug !== 'activity-depth' && slug !== 'wellbeing' && slug !== 'platform' && slug !== 'revenue' && slug !== 'ai-usage' && slug !== 'complaints' && slug !== 'bookings' && pageCache[slug]) {
    ...
    fetch('pages/' + (entry.file || slug) + '.html', { cache: 'no-cache' })
      .then(function(res){
  ...
  function renderForbidden(slug) {
    ...
    slot.innerHTML =
      '<div class="page-hero">' +
        ...
        '<div style="margin-top:18px;display:flex;gap:10px"><a class="btn btn-primary" href="#/home">Back to Home</a></div>' +
      '</div>';
  }
  ...
  function renderNotFound(slug) {
    ...
    if (crumbsHost) crumbsHost.innerHTML = '<a class="crumb" href="#/home">VYVE</a> <span class="crumb-sep">/</span> <span class="crumb crumb-current">Not found</span>';

  ...
  (function(){
    ...
    st.textContent =
      ...
      '.exv2-modal .in{max-width:1400px;height:92vh;}' +
      '.exv2-modal .in .sc{flex:1;}' +
      '.exv2-modal .exv2-rail{max-height:calc(92vh - 130px);}' +
  ...
  exLoad = async function(){ var r = await _exv2Load.apply(this, arguments); if (exv2.built) exv2RailPaint(); return r; };
  ...
  renderDayEditor = function(container, day){
    ...
    [['de-add-warm', 'de-warm', 'Warm up', { sets: '1', reps: '', rest_seconds: 0 }], ['de-add-main', 'de-main', 'Workout', {}], ['de-add-cool', 'de-cool', 'Cool down', { sets: '1', reps: '', rest_seconds: 0 }]].forEach(function(spec){
      var btn = container.querySelector('.' + spec[0]); if (!btn || btn.nextElementSibling && btn.nextElementSibling.classList.contains('exv2-choose')) return;
      var b = document.createElement('button');
      b.type = 'button'; b.className = 'btn btn-primary exv2-choose'; b.style.cssText = 'font-size:11.5px;padding:5px 10px;margin-left:6px;';
      b.textContent = '\u2630 Choose from library';
      b.addEventListener('click', function(){ exv2PickerOpen(container.querySelector('.' + spec[1]), spec[2], spec[3]); });

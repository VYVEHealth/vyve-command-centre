  ...
  (function(){
    ...
    st.textContent =
      ...
      '.exv2-modal{justify-content:flex-end;align-items:stretch;padding:0;}' +
      '.exv2-modal .in{max-width:min(1180px,88vw);width:100%;height:100vh;max-height:100vh;border-radius:14px 0 0 14px;transform:translateX(100%);transition:transform .22s ease;}' +
  ...
  renderDayEditor = function(container, day){
    ...
    if (!day || !day.exercises || !day.exercises.length){
      ...
      if (mm && mm.children.length === 1){ var only = mm.children[0], nm = only.querySelector('.de-name'); if (nm && !nm.value) only.remove(); }
    }
    ...
    [['de-add-warm', 'de-warm', 'Warm up', { sets: '1', reps: '', rest_seconds: 0 }], ['de-add-main', 'de-main', 'Workout', {}], ['de-add-cool', 'de-cool', 'Cool down', { sets: '1', reps: '', rest_seconds: 0 }]].forEach(function(spec){
      ...
      link.type = 'button'; link.className = 'btn exv2-blank'; link.style.cssText = 'font-size:11px;padding:5px 8px;margin-left:6px;border-color:transparent;color:var(--text-muted);';
      link.textContent = 'or type a name';
      link.addEventListener('click', function(){ addDayRow(container.querySelector('.' + spec[1]), Object.assign({}, spec[3])); var host = container.closest('[data-w3-tvs-host]') || container; if (typeof w3TvsRefresh === 'function') w3TvsRefresh(host); });
      fresh.parentElement.insertBefore(link, fresh.nextSibling);
    });
  };
  ...
  document.addEventListener('click', function(ev){
    ...
    var isDur = d.dataset.w3type === 'duration';
    w3RowSetMode(d, !isDur);
    var inp = f.querySelector('.de-reps');
    if (!isDur){ if (!/^\d+$/.test(inp.value)) inp.value = '30'; } else { if (/^\d+$/.test(inp.value)) inp.value = '8-12'; }
